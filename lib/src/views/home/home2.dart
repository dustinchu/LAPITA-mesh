...
class Home2 extends StatefulWidget {
  ...
  const Home2({Key? key, required this.nordicNrfMesh}) : super(key: key);
...
class _Home2State extends State<Home2> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((mag) async {
      widget.nordicNrfMesh.meshManagerApi.loadMeshNetwork();
    });
  }
            ...
            onPressed: _meshNetwork != null
                ? () async {
                    ...
                    if (groupName != null && groupName.isNotEmpty) {
                      try {
                        await _meshManagerApi.meshNetwork!
                      ...
                      scaffoldMessenger.showSnackBar(const SnackBar(
                          content: Text('No name given, aborting')));
                    }
                  }
                : null,
...
class PlatformVersion extends StatefulWidget {
  const PlatformVersion({Key? key, required this.nordicNrfMesh})
      : super(key: key);
          ...
          onPressed: meshNetwork != null
              ? () async {
                  final meshNetworkJson =
                      await meshManagerApi.exportMeshNetwork();
...
class MeshNetworkManagerWidget extends StatelessWidget {
  ...
  const MeshNetworkManagerWidget({Key? key, required this.nordicNrfMesh})
      : super(key: key);
        ...
        TextButton(
          onPressed: meshNetwork != null
              ? () async {
                  ...
                  final result = await meshNetwork.addProvisioner(
                      0x0888, 0x02F6, 0x0888, 5);
          ...
          onPressed: meshNetwork != null
              ? () async {
                  ...
                  if (groupAdr != null && groupAdr.isNotEmpty) {
                    try {
                      await meshManagerApi.meshNetwork!
                          .removeGroup(int.parse(groupAdr));
                      scaffoldMessenger
                          .showSnackBar(const SnackBar(content: Text('OK')));
                    } on PlatformException catch (e) {
                      scaffoldMessenger.showSnackBar(
                    ...
                    scaffoldMessenger.showSnackBar(const SnackBar(
                        content: Text('No address given, aborting')));
                  }
                }
              : null,
        ...
        TextButton(
          onPressed: meshNetwork != null
              ? () async {
                  ...
                  if (groupAdr != null && groupAdr.isNotEmpty) {
                    try {
                      final subs = await meshManagerApi.meshNetwork!
                          .elementsForGroup(int.parse(groupAdr));
                      debugPrint('$subs');
                      scaffoldMessenger
                          .showSnackBar(const SnackBar(content: Text('OK')));
                    } on PlatformException catch (e) {
